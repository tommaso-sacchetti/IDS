...
def _filter_blacklisted_id(dataset: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    if os.path.exists(whitelist_file) and os.stat(whitelist_file).st_size != 0:
        ...
        if DEBUG:
            print(
                f"Dataset shape: {dataset.shape}; whitelisted_dataset shape: {whitelisted_dataset.shape}; blacklisted_dataset shape: {blacklisted_dataset.size}"
            )
            print(
                f"IDs in whitelist: {len(whitelisted_dataset['id'].unique())}; IDs in blacklist: {len(blacklisted_dataset['id'].unique())}"
            )
        ...
        if id in id_periods:
            ...
            for index, period in enumerate(periods):
                if period > range_max or period < range_min:
                    ...
                    if DEBUG:
                        print(
                            f"ID: {id}, MIN: {range_min}, MAX: {range_max}, PERIOD {period}"
                        )
                    row = df.iloc[[index]]
